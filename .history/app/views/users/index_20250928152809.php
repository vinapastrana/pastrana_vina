...
<body class="bg-gradient-to-br from-pink-200 via-pink-100 to-pink-300 min-h-screen font-sans text-gray-800">
  ...
  <!-- Main Content -->
  <div class="max-w-6xl mx-auto mt-10 px-4 relative">

    <!-- Logout Button (top-right inside content) -->
    <div class="absolute top-0 right-0 mt-4 mr-4">
      <a href="<?=site_url('reg/logout');?>"
         class="bg-pink-500 hover:bg-pink-600 text-white font-medium px-4 py-2 rounded-lg shadow transition duration-200">
         Logout
      </a>
    </div>
    ...
    <div class="bg-white bg-opacity-90 backdrop-blur-sm shadow-xl rounded-2xl p-8">
      ...
      <?php if(!empty($logged_in_user)): ?>
        <div class="mb-8 bg-pink-100 text-pink-800 px-6 py-5 rounded-xl shadow-lg text-center">
          ...
          <p class="text-lg md:text-xl">Role: <span class="font-semibold"><?= html_escape($logged_in_user['role']); ?></span></p>
        </div>
      <?php else: ?>
